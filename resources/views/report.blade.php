                    ...
                    <div class="about__dates flex">
                        @foreach ($reportYears as $key => $val)
                            <div class="governing__date date-12">
                                <a aria-label="{{ $key }}" class="date about__date @if($year == $key)date-active @endif" href="{{ route('report', ['year' => $key]) }}">{{ $key }}</a>
                            </div>
                        @endforeach
                    </div>
                </div>
            ...
            <div class="wrapper">
                <div class="pdfs__cont">
                    @foreach ($reports as $report)
                        <div>
                            <a aria-label="{{ $report->{'name_'.app()->getLocale()} }}" class="report__link text-18" href="{{ Storage::url($report->pdf) }}" target="_blank">{{ $report->{'name_'.app()->getLocale()} }}</a>
                        </div>
                    @endforeach
                </div>
            </div>
